...
loadTextures({
  ...
  "avatar_human": "skyhighheroes:transer/geo_stelar_head",
  "avatar_transformed": "skyhighheroes:transer/mega_man_head",
  ...
  "mask": "skyhighheroes:geo/mega_man_mask.tx.json",
  "mask_lights": "skyhighheroes:geo/mega_man_mask_lights.tx.json",
  "mask_wave_changing_lights": "skyhighheroes:geo/mega_man_mask_wave_changing_lights.tx.json",
  ...
  renderer.setTexture((entity, renderLayer) => {
    if (renderLayer == "CHESTPLATE") {
      ...
      if (!hasEMBeing(entity)) {
        if (entity.getInterpolatedData("skyhighheroes:dyn/wave_calling_timer") > 0.45 && entity.getInterpolatedData("skyhighheroes:dyn/wave_calling_timer") < 0.6) {
          return "base";
        };
      };
...
function initEffects(renderer) {
  ...
  callingForcefield.setCondition(entity => {
    return !entity.is("DISPLAY") && !hasEMBeing(entity) && entity.getData("skyhighheroes:dyn/wave_calling") && (entity.getInterpolatedData("skyhighheroes:dyn/wave_changing_timer") == 0);
  });
  ...
  barrierForcefield.setCondition(entity => {
    barrierForcefield.opacity = ((entity.getData("skyhighheroes:dyn/battle_card") == 1) ? 1.0 : 0.0) * entity.getInterpolatedData("fiskheroes:shield_blocking_timer") * 0.1;
    return !entity.is("DISPLAY") && hasEMBeing(entity) && (entity.getInterpolatedData("skyhighheroes:dyn/wave_changing_timer") == 1) && (barrierForcefield.opacity > 0.0);
  });
  ...
  stelar.initMegaBuster(renderer, getBusterColor(), getMainColor());
  stuff.bindBeam(renderer, "fiskheroes:repulsor_blast", "fiskheroes:repulsor_blast", "rightArm", getMainColor(), [
  ...
  stuff.bindFlightTrail(renderer, getTrail());
  //Cannon
...
function initAnimations(renderer) {
  stuff.addAnimationWithData(renderer, "transer.CALLING", "skyhighheroes:stelar_calling", "skyhighheroes:dyn/wave_calling_timer").setCondition(entity => {return entity.getData("skyhighheroes:dyn/wave_calling")});
  stelar.initHoloFlightAnim(renderer, "wave.HOLOGRAM_FLIGHT", "skyhighheroes:em_wave_change_holo_flight", "Omega-Xis");
...
function render(entity, renderLayer, isFirstPersonArm) {
  ...
  if (!hasEMBeing(entity)) {
    if (entity.is("DISPLAY")) {
      ...
      var callingTimer = entity.getInterpolatedData("skyhighheroes:dyn/wave_calling_timer");
      ...
      if (callingTimer > 0 && entity.getData("skyhighheroes:dyn/wave_calling") && callingTimer < 0.5) {
        callingBeam.render();
      };
      if (entity.getInterpolatedData("skyhighheroes:dyn/wave_calling_timer") > 0.45 && entity.getInterpolatedData("skyhighheroes:dyn/wave_calling_timer") < 0.6) {
        callingHead.render();
    ...
    if (!entity.is("DISPLAY")) {
      barrierForcefield.opacity = (entity.getData("skyhighheroes:dyn/battle_card") == 1) * entity.getInterpolatedData("fiskheroes:shield_blocking_timer") * 0.1;
      if (entity.getInterpolatedData("skyhighheroes:dyn/wave_changing_timer") == 0) {
        visualizerModel.render();
      };
    ...
    if (isChristmasSeason) {
      if (!entity.is("DISPLAY") && entity.getInterpolatedData("skyhighheroes:dyn/wave_changing_timer") == 0) {
        santaHat.setScale(1.05);
        santaHat.setOffset(0.0, -5.75, 1.25);
        santaHat.setRotation(-45.0, 0.0, 0.0);
        if (entity.getData("skyhighheroes:dyn/visualizer_toggle")) {
  ...
  if (!entity.is("DISPLAY") && renderLayer == "CHESTPLATE" && hasEMBeing(entity)) {
    if (entity.getInterpolatedData("skyhighheroes:dyn/wave_changing_timer") == 1 && entity.getInterpolatedData("fiskheroes:mask_open_timer2") > 0) {
  ...
  if (entity.is("DISPLAY")) {
    ...
    if (hasEMBeing(entity) && (entity.as("DISPLAY").getDisplayType() == "HOLOGRAM" || entity.as("DISPLAY").getDisplayType() == "DISPLAY_STAND" || entity.as("DISPLAY").getDisplayType() == "ITERATOR_PREVIEW")) {
      maskWaveChangingLights.render();
      mask.render();
...
function hasEMBeing(entity) {
  return ((entity.is("DISPLAY")) ? (stelar.mainNBT(entity).getString("emBeing") == "Omega-Xis") : (entity.getData("skyhighheroes:dyn/em_being") == "Omega-Xis"));
};
...
function getTrail() {
  return "skyhighheroes:mega_man_flight";
};